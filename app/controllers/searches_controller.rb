class SearchesController < ApplicationController
  ...
  def create
    ...
    Search.create(user_ip_address:, query:, completed: false)
  ...
  def resolve_queries
    ...
    saved_query = Search.find_by(query: final_query, user_ip_address:, completed: true)
    if saved_query
      saved_query.count += 1
      saved_query.save
      new_query = saved_query
    else
      new_query = Search.create(user_ip_address:, query: final_query, completed: true, count: 1)
    end
    @incomplete_searches.destroy_all
    render json: { status: '200', new_query: }
  end
